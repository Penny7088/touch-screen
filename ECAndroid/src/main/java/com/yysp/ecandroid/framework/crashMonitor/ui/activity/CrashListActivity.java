...
import com.yysp.ecandroid.framework.crashMonitor.ui.adapter.CrashInfoAdapter;
import com.yysp.ecandroid.framework.crashMonitor.ui.adapter.OnItemClickListener;
import com.yysp.ecandroid.framework.crashMonitor.ui.utils.DateUtil;
...
public class CrashListActivity extends CrashBaseActivity implements SwipeRefreshLayout.OnRefreshListener, SearchView.OnQueryTextListener {
    ...
    private void getCrashList() {
        //重新获取
        File fileCrash = new File(FileUtils.getCrashLogPath(context));
        fileList = FileUtils.getFileList(fileCrash);
        ...
        Collections.sort(fileList, new Comparator<File>() {
            ...
            public int compare(File file01, File file02) {
                ...
                if (file01_array.length > 1 && file02_array.length > 1) {
                    ...
                    try {
                        //时间转换为毫秒
                        int time01 = Integer.parseInt(DateUtil.dataToTime(date01));
                        int time02 = Integer.parseInt(DateUtil.dataToTime(date02));
                        if (time01 > time02) {
                            return -1;
                        } else {
                            return 1;
