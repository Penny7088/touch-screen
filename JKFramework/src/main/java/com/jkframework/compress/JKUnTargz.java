...
public class JKUnTargz extends JKBaseUncompress{
	...
	{		
		...
		try {
			taisStream = new TarArchiveInputStream(new GzipCompressorInputStream(new FileInputStream(tFilepath)),tEncoding);
			int nFileCount = 0;
			...
			e.printStackTrace();
			JKLog.ErrorLog("打开SD卡上tar.gz文件失败.原因为:" + e.getMessage());
			return -1;
		}
		return nTotal;
	}
	...
	{		
		...
		this.tEncoding = tEncoding;
		try {
			taisStream = new TarArchiveInputStream(new GzipCompressorInputStream(JKFile.ReadAssets(tAssetsName)),tEncoding);
			int nFileCount = 0;
			TarArchiveEntry taeEntry;
			while((taeEntry = taisStream.getNextTarEntry()) != null){
				//如果条目是文件目录，则继续执行
			...
			e.printStackTrace();
			JKLog.ErrorLog("打开Asset上的tar.gz文件失败.原因为:" + e.getMessage());
			return -1;
		}
		return nTotal;
	}
	...
	public void Reset()
	{
		try {
			if (tFilePath.charAt(0) == '/')
				...
				taisStream = new TarArchiveInputStream(new GzipCompressorInputStream(JKFile.ReadAssets(tFilePath)),tEncoding);
		} catch (IOException e) {
			e.printStackTrace();
			JKLog.ErrorLog("打开tar.gz文件失败.原因为:" + e.getMessage());
		}
	}
	...
	public void Close()
	{
		try {			
			...
			e.printStackTrace();
			JKLog.ErrorLog("关闭tar.gz流失败.原因为:" + e.getMessage());
		}
